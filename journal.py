...
diary_note=""
option=st.sidebar.selectbox("select an option",["search diary","add diary"],key="sidebar_option")
if option=="add diary":
        ...
        if st.button("search",key="search",type="primary",use_container_width=True,width="stretch`"):
            ...
            if os.path.exists(filename):
                with open(filename,'r',encoding='utf-8') as f:
                    diary_note=f.read()
    if diary_note:
        st.text_area("diary note on " + search_date.strftime("%d-%m-%y"),value=diary_note,height=200)
    else:
